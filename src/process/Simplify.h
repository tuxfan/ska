...
public:
  ...
  simplify_nodes(std::map<llvm::Value *, intf> intf_table,
    std::vector<register_set_shared_t> & rs);

  void num_phys_regs(std::vector<register_set_shared_t> & rs);
...
void simplify_nodes::num_phys_regs(std::vector<register_set_shared_t> & rs) {

  for(auto ita: rs) {
    reg_map[ita->type()] = ita->num_registers();
  } // for
...
simplify_nodes::simplify_nodes(std::map<llvm::Value *, intf> intf_table,
  ...
  num_phys_regs(rs) ;  // gets data from reg set
